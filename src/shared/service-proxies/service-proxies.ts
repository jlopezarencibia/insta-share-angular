...
export class UserServiceProxy {
     ...
     * @return Success
     */
    updateProfile(body: UserLoginInfoDto | undefined): Observable<UserLoginInfoDto> {
        let url_ = this.baseUrl + "/api/services/app/User/UpdateProfile";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("put", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<UserLoginInfoDto>;
                }
            } else
                return _observableThrow(response_) as any as Observable<UserLoginInfoDto>;
        }));
    }

    protected processUpdateProfile(response: HttpResponseBase): Observable<UserLoginInfoDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = UserLoginInfoDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => {
...
export class UserFileServiceProxy {
     ...
     * @return Success
     */
    rename(fileId: number | undefined, newName: string | undefined): Observable<BasicUaserFileDto> {
        let url_ = this.baseUrl + "/api/services/app/UserFile/Rename?";
        if (fileId === null)
            throw new Error("The parameter 'fileId' cannot be null.");
        else if (fileId !== undefined)
            url_ += "fileId=" + encodeURIComponent("" + fileId) + "&";
    ...
    protected processRename(response: HttpResponseBase): Observable<BasicUaserFileDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
